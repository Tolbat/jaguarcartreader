...
//#define EnableSerialDebug // serial monitor output [COMMENT OUT to disable]
//******************************************************************************
...
unsigned long cartSize; // (0x20000)/0x100000/0x200000/0x400000/0x600000
byte romSize = 0; // 0 = 1MB, 1 = 2MB, 2 = 4MB, 3 = 6MB, 4 = 4M16, 5 = 6M16
...
static const char menuItem7[] PROGMEM = "Analyze Cartridge";
...
static const char romItem4[] PROGMEM = "6MB ROM";
static const char romItem5[] PROGMEM = "4M16 ROM";
static const char romItem6[] PROGMEM = "6M16 ROM";
static const char* const romMenu[] PROGMEM = {romItem1, romItem2, romItem3, romItem4, romItem5, romItem6};
...
void menu() {
  ...
  Serial.println(F("(6) Analyze Cartridge"));
  ...
  switch (answer) {
    ...
    // Analyze cartridge bus behavior (read-only diagnostic build)
    case 6:
      clear();
      debug_println(F("6 (Analyze Cartridge)\n"));
      sd.chdir("/");
      analyzeCartridge();
      break;
...
void sizeMenu() {
  ...
  Serial.println(F("(3) 6MB ROM"));
  Serial.println(F("(4) 4M16 ROM"));
  Serial.println(F("(5) 6M16 ROM"));
  Serial.println("");
  Serial.print("Enter Command: ");
#endif
  convertPgm(romMenu, 6);
  unsigned char subMenu = menu_box("Select ROM Size", menuChoices, 6, 0);
...
void setup() {
  ...
  // Print all the info
  if (romSize > 5) { // ROM mode out of range, set to default
...
//******************************************************************************
// READ-ONLY CARTRIDGE ANALYZER
//******************************************************************************
// oeMode: 0 = both OE lines, 1 = OEL only, 2 = OEH only
void readDataMode(unsigned long myAddress, byte oeMode) {
  SRCLR_CLEAR;
  SRCLR_SET;
  LATCH_CLEAR;
  ...
  // Preserve the normal reader's address setup time.
  __asm__("nop\n\t");

  // Begin each cycle with both output enables inactive.
  PORTH |= (1 << 3) | (1 << 4);

  // Assert cartridge chip enable.
  PORTH &= ~(1 << 5);

  if (oeMode == 1) {
    // OEL only: PH4 low, PH3 high.
    PORTH &= ~(1 << 4);
  }
  else if (oeMode == 2) {
    // OEH only: PH3 low, PH4 high.
    PORTH &= ~(1 << 3);
  }
  else {
    // Normal 32-bit read: both output enables active.
    PORTH &= ~(1 << 3) & ~(1 << 4);
  }

  // Preserve the normal reader's sample delay.
  __asm__("nop\n\t""nop\n\t""nop\n\t""nop\n\t""nop\n\t""nop\n\t""nop\n\t""nop\n\t""nop\n\t""nop\n\t""nop\n\t""nop\n\t");

  tempDataLO = (((PINK & 0xFF) << 8) | (PINF & 0xFF));
  tempDataHI = (((PINC & 0xFF) << 8) | (PINL & 0xFF));

  // End the read cycle. WE remains inactive throughout this function.
  PORTH |= (1 << 3) | (1 << 4);
  PORTH |= (1 << 5);
  __asm__("nop\n\t""nop\n\t""nop\n\t""nop\n\t""nop\n\t""nop\n\t");
...
void writeHexDigit(byte value) {
  if (value < 10)
    myFile.write((byte)('0' + value));
  else
    myFile.write((byte)('A' + value - 10));
}
...
void writeHex24(unsigned long value) {
  int shift;
  for (shift = 20; shift >= 0; shift -= 4)
    writeHexDigit((value >> shift) & 0x0F);
}

void analyzeAddress(unsigned long address) {
  byte mode;
  byte repeat;

  for (mode = 0; mode < 3; mode++) {
    for (repeat = 0; repeat < 4; repeat++) {
      readDataMode(address, mode);

      writeHex24(address);
      myFile.write((byte)',');
      myFile.write((byte)('0' + mode));
      myFile.write((byte)',');
      myFile.write((byte)('0' + repeat));
      myFile.write((byte)',');
      writeHex16(tempDataHI);
      myFile.write((byte)',');
      writeHex16(tempDataLO);
      myFile.write((byte)'\n');
    }
  }
}

void analyzeCartridge() {
  ...
  // Create a separate numbered report folder.
  sd.chdir();
  EEPROM_readAnything(0, foldern);
  sprintf(folder, "JAG/ANALYZE/%d", foldern);
  ...
  myFile.println(F("JAGUAR CARTRIDGE READ-ONLY ANALYSIS"));
  myFile.println(F("ADDRESS,MODE,REPEAT,DATA_HI,DATA_LO"));
  myFile.println(F("MODE 0=BOTH, MODE 1=OEL ONLY, MODE 2=OEH ONLY"));
  myFile.println(F(""));

  clear();
  println_Msg(F("Analyzing cartridge"));
  println_Msg(F("READ ONLY"));
  display_Update();

  // Exercise A0/A1 and nearby address behavior at the start of ROM.
  for (address = 0; address < 0x40; address++)
    analyzeAddress(address);

  // Sample the start of each 1MB region within the selected size.
  for (base = 0; base < cartSize; base += 0x100000) {
    for (offset = 0; offset < 16; offset++)
      analyzeAddress(base + offset);
  }

  // Sample the final 16 addresses in the selected range.
  if (cartSize >= 16) {
    for (address = cartSize - 16; address < cartSize; address++)
      analyzeAddress(address);
  }
  ...
  clear();
  println_Msg(F("Analysis saved:"));
  println_Msg(F("ANALYZE.TXT"));
#ifdef OLED
  display.setCursor(0, 56);
  display.println(F("Press Button..."));
#endif
...
//  sprintf(folder, "JAG/ROM/%s/%d", romName, foldern);
  ...
  for (unsigned long currBuffer = 0; currBuffer < cartSize; currBuffer += 512) {
    ...
    if ((romSize == 4) || (romSize == 5)) {
      // 16-bit cartridge: A0 is unused, each new word is at address +2.
      // The ROM data is on D0-D15; D16-D31 is not written to the dump.
      for (int currWord = 0; currWord < 512; currWord += 2) {
        readData(currBuffer + currWord);
        sdBuffer[d] = (tempDataLO >> 8) & 0xFF;
        sdBuffer[d + 1] = tempDataLO & 0xFF;
        d += 2;
      }
    }
    else {
      // Standard 32-bit Jaguar MaskROM read.
